...
import { useUploadUserCoverMutation } from '@/lib/redux/api/admin-api';

export default function TestUploadPage() {
  const [file, setFile] = useState<File | null>(null);
  const [preview, setPreview] = useState<string | null>(null);
  const [userId, setUserId] = useState(''); // nhập userId để test
  const [uploadUserCover, { isLoading, error, data }] =
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      alert('Upload thành công: ' + result.coverUrl);
    } catch (err: any) {
      console.error(err);
    ...
    <div style={{ padding: 20 }}>
      ...
      {data && (
        <div style={{ color: 'green', marginTop: 10 }}>
          Success! Cover URL: {data.coverUrl}
        </div>
      )}
    </div>
  );
