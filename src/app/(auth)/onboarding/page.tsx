  ...
  const handleNext = async (stepData?: any) => {
    ...
    try {
      ...
      if (currentStep === STEP_CONFIG.length) {
        ...
        // Make sure step 4 (preferences) is marked as completed
        await updateProgress({
          step: 4,
          data: {
            preferences: {
        ...
        // Only try to select path if it's a real database path, not an assessment recommendation
        const isAssessmentRecommendation =
          selectedPath.courses?.length === 1 &&
          selectedPath.courses[0].id === selectedPath.id;

        if (!isAssessmentRecommendation) {
          // Select learning path first for real database paths
          await selectPath({
        ...
        // Mark step 5 (learning path selection) as completed
        await updateProgress({
          step: 5,
          data: {
        ...
        // Set tokens to localStorage like login does
        if (result.accessToken && result.refreshToken) {
          ...
          // Also dispatch loginSuccess to trigger auth middleware
          if (result.user) {
  ...
  const canProceed = () => {
    const result = (() => {
      switch (currentStep) {
        ...
        case 3: // Skill assessment step
          // Allow proceeding immediately when assessment is completed
