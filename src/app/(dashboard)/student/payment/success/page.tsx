...
import { useGetPaymentByOrderCodeQuery } from '@/lib/redux/api/ecommerce-api';
...
export default function PaymentSuccess() {
  ...
  const orderId = searchParams.get('orderCode') || searchParams.get('orderId');

  const {
    data: paymentData,
    isLoading,
    error,
  } = useGetPaymentByOrderCodeQuery(orderId!, {
    skip: !orderId,
  });
  ...
  useEffect(() => {
    ...
    if (paymentData) {
      ...
      setPurchaseDetails({
        ...
        transactionId: searchParams.get('session_id') || paymentData.id,
        items:
          paymentData.items?.map((item: any) => ({
            id: item.id,
            courseId: item.courseId,
            courseName: item.courseTitle || 'Course',
        ...
        paymentMethod:
          paymentData.paymentMethod === 'stripe' ? 'Stripe' : 'MoMo',
  ...
  }, [orderId, paymentData, error, router, searchParams]);
